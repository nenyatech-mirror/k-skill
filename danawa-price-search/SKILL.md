...
AJAX endpoint는 HTML fragment를 반환한다. helper는 `.diff_item`, 쇼핑몰 로고 `alt`, `em.prc_c`/`em.prc_t`, 배송 문구, 결제조건 배지(`.ico.cash`/`.ico.point`/`.ico.coupon`/`.ico.card` 등), 카드 할인 라인, 무이자 할부 레이어, 다나와 bridge link를 파싱한다.
...
`offers[]`는 **배송비 포함 실구매가(`total_price`) 오름차순**으로 정렬된다. 결제조건(현금/쿠폰/포인트/특정카드 한정)이 붙은 row도 같은 정렬에 그대로 참여한다 — 가장 싸면 1위로 올라온다. 결제조건은 분리 그룹이나 추가 필터링 없이 row 단위 `payment_badges` / `cash_only` / `point_only` / `coupon_only` / `card_only_badge` / `is_conditional_price` 필드로 노출한다. 호출자는 사용자의 결제 수단에 따라 직접 판단한다.
...
- `installment`
- `installment_detail`
- `payment_badges` — Danawa가 가격 옆에 노출한 결제조건 배지 텍스트 목록 (예: `["현금"]`, `["포인트"]`, `["쿠폰"]`)
- `cash_only` — 현금 결제 전용가
- `point_only` — 포인트 차감 적용가
- `coupon_only` — 쿠폰 적용가
- `card_only_badge` — 특정 카드 한정 노출가
- `is_conditional_price` — 위 중 하나라도 True. **일반 결제가가 아니므로 카드 일반 결제 시 가격이 다르거나 불가능할 수 있음**
- `url`

항상 무료배송 여부, 배송비 포함 실구매가, 카드별 할인 가격, 무이자 할부 문구, **그리고 `payment_badges`/`is_conditional_price`를 함께 확인한다.** 조건부 가격을 일반가처럼 1위로 노출하면 비교 결과가 거짓이 된다.
...
1. **`total_price` 오름차순 단일 기준.** 결제조건(현금/쿠폰/포인트/특정카드 한정)이 붙은 row도 같은 정렬에 그대로 참여한다 — 가장 싸면 1위로 올라온다. 결제조건은 분리 그룹화하지 않고 표의 "결제조건" 컬럼에 행별로 표시한다 (`cash_only=True` → "현금 전용", `coupon_only=True` → "쿠폰 적용가", `point_only=True` → "포인트 적용가", `card_only_badge=True` → 카드명 명시, 그 외 → "일반"). 사용자는 자기 결제 수단에 따라 직접 판단한다.
...
- 다나와가 새로운 결제조건 배지 클래스나 문구를 도입하면 결제조건 배지 화이트리스트(`cash`/`point`/`coupon`/`discount`/`card`/`membership` 클래스, `현금`/`포인트`/`쿠폰`/`할인` 텍스트 키워드)를 갱신해야 한다.
